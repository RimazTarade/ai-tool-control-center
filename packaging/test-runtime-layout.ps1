...
$tempDir = Join-Path ([System.IO.Path]::GetTempPath()) ("ai-tool-control-runtime-layout-test-" + [guid]::NewGuid())
...
Set-Content -Path (Join-Path $archiveSource "not-python.txt") -Value "invalid layout"
  ...
  "runtimes": [
    {
      ...
      "sha256": "$hash",
      "stage_directory": "cpython-layout-test"
    }
  ]
}
...
try {
    ...
    if ($_.Exception.Message -notmatch "runtime layout") {
        throw "Expected runtime layout rejection, got: $($_.Exception.Message)"
    }
}
finally {
...
Write-Host "PASS: invalid CPython runtime layout is rejected."
